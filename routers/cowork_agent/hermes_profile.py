...
async def _run_default_hermes_sh(subcommand: str, timeout_s: float) -> tuple[int, str]:
    """Delegate default-profile gateway lifecycle to hermes.sh (which
    is what actually manages the default gateway). Imported lazily to
    avoid importing channels.py at module load (it has its own state)."""
