...
codex agents are project folders under xo-projects (shared across backends);
their record lives in ``<project>/.xo/agent.json``. The core router forwards here
via ``load_capability('agents', …)`` instead of branching on the backend name.
Structurally identical to the other adapters' agents capability — only the
``backend`` tag differs. codex ``$skill`` prefixes are chat-routing concerns, NOT
sidebar agents, so they are deliberately not modelled here.
"""
...
def create_agent(body) -> dict | JSONResponse:
    """Create a codex agent: scaffold the project tree + write the record."""
    ...
    if _load(agent_id) is not None:
        return JSONResponse(
            status_code=409,
            content={"detail": f'Codex agent "{agent_id}" already exists.'},
        )
...
def delete(agent_id: str) -> dict | JSONResponse | None:
    """codex has no delete contract today (parity with the other adapters)."""
    return None
