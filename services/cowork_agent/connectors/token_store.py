...
provider-keyed entry in one shared JSON file at ~/.config/xo-cowork/mcp-tokens.json.
This module is the ONLY place that knows the file's location, its on-disk shape,
and its read/write semantics. Connectors get/set/delete by provider key and
never touch the format — so locking or a format migration can later be added
here once, not in every connector.
"""
...
from .config_paths import TOKEN_FILE, ensure_connector_config_dir, ensure_token_file_migrated
