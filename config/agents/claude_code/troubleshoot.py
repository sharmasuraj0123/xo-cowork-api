#!/usr/bin/env python3
"""Check the live claude_code install against its commands.json paths.

Reads:
  - sibling commands.json     (canonical claude paths)
  - ~/.claude/settings.json   (what the claude CLI actually has configured)
  - ~/.claude/.env            (api keys / secrets, when present)
...
def check_config_file(report: Report, paths: dict[str, Path]) -> None:
    ...
    if err:
        report.add(FAIL, "settings.json readable", err)
        return
    report.add(OK, "settings.json readable", str(cfg_path))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Troubleshoot the claude_code agent install.")
    parser.parse_args()
    ...
    binary = commands.get("binary") or commands.get("name") or "claude"
    paths = {
