...
Perform code reviews on local changes before pushing, or review open PRs on GitHub. Most of this skill uses plain `git`; the `gh`/`curl` split only matters for PR-level interactions. The token comes from `mcp-tokens.json` (see `github-auth`).
...
When reviewing local changes, present findings in this structure (see `references/review-output-template.md`):
