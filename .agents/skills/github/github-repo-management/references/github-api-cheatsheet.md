...
All requests need: `-H "Authorization: token $GH_TOKEN"`

Use the `gh-env.sh` helper to set `$GH_TOKEN`, `$GH_OWNER`, `$GH_REPO` automatically (token comes from `mcp-tokens.json`):
```bash
...
- Check remaining: `curl -s -H "Authorization: token $GH_TOKEN" https://api.github.com/rate_limit`
