...
Create, search, triage, and manage GitHub issues. Each section shows `gh` first, then the `curl` REST equivalent. The token comes from `mcp-tokens.json` (see `github-auth`).
...
Source the helper once — it exports `GH_TOKEN`, `GH_USER`, and (inside a repo) `GH_OWNER` / `GH_REPO` / `GH_OWNER_REPO`:

```bash
source "${GH_SKILL_DIR:-/home/coder/xo-cowork-api/.agents/skills/github}/github-auth/scripts/gh-env.sh"
```

If you are not inside the target repo, set `GH_OWNER` / `GH_REPO` yourself or pass `--repo owner/name` to `gh`.
...
curl -s \
  -H "Authorization: token $GH_TOKEN" \
  "https://api.github.com/repos/$GH_OWNER/$GH_REPO/issues?state=open&labels=bug&per_page=20" \
  | python3 -c "
import sys, json
for i in json.load(sys.stdin):
...
curl -s \
  -H "Authorization: token $GH_TOKEN" \
  "https://api.github.com/search/issues?q=authentication+error+repo:$GH_OWNER/$GH_REPO" \
  | python3 -c "
import sys, json
...
Issue body templates live in `templates/bug-report.md` and `templates/feature-request.md`.
...
curl -s -X DELETE \
  -H "Authorization: token $GH_TOKEN" \
  https://api.github.com/repos/$GH_OWNER/$GH_REPO/issues/42/labels/needs-triage
...
curl -s -X POST \
  -H "Authorization: token $GH_TOKEN" \
  https://api.github.com/repos/$GH_OWNER/$GH_REPO/issues/42/comments \
...
curl -s \
  -H "Authorization: token $GH_TOKEN" \
  "https://api.github.com/repos/$GH_OWNER/$GH_REPO/issues?labels=needs-triage&state=open" \
  | python3 -c "
import sys, json
for i in json.load(sys.stdin):
    if 'pull_request' not in i:
